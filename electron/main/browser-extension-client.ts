...
export class BrowserExtensionClient {
  ...
  private receive(message:any){this.changed({lastCommunication:new Date().toISOString()});if(message?.event==="stream_closed"){this.changed({lastClosedStream:message.payload});return;}const item=this.pending.get(message?.requestId);if(!item)return;clearTimeout(item.timer);this.pending.delete(message.requestId);if(message.success)item.resolve(message.payload);else item.reject(new Error(message.error??"Extensión rechazada."));}
