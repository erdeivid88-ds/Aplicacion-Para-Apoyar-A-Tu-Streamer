...
function traceManaged(stage:string,m:any,details:Record<string,unknown>={}){console.info("[managed-tab-trace]",{stage,traceId:m?.payload?.traceId,appSessionId:m?.appSessionId,...details});}
...
async function exact(m:any) { const tabId=m.payload.tabId;const cached=typeof tabId==="number"?managed.get(tabId):undefined;traceManaged("I_MANAGED_LOOKUP",m,{tabId,tabIdType:typeof tabId,managedTabIds:[...managed.keys()],entryFound:Boolean(cached),storedAppSessionId:cached?.appSessionId,receivedAppSessionId:m.appSessionId});try{const managedTab=await registry.getManagedTab(tabId,m.appSessionId);const {item,tab}=managedTab;if(m.action!=="configure_audio"&&(item.streamerId!==m.payload.streamerId||item.platform!==m.payload.platform||item.streamSessionId!==m.payload.streamSessionId||item.monitorSessionId!==m.payload.monitorSessionId))throw new Error("TAB_NOT_REGISTERED");traceManaged("I_MANAGED_LOOKUP_RESULT",m,{tabId,result:"FOUND",storedAppSessionId:item.appSessionId});const ready=item.platform==="kick"?await readyKickTab(tabId):undefined;return{item,tab:ready?.tab??tab,diagnostic:ready?.diagnostic};}catch(error){traceManaged("I_MANAGED_LOOKUP_RESULT",m,{tabId,result:"REJECTED",errorCode:error instanceof Error?error.message:"rejected"});throw error;}}
...
async function onMessage(m:any) {
  try {
    if (m?.action === "handshake") { validate(m,true); inactive(); requestIds.add(m.requestId); applicationConnected=true; appSessionId=m.appSessionId; lastHeartbeat=Date.now(); armWatchdog(); response(m,true,{extensionVersion:chrome.runtime.getManifest().version,managedRegistryVersion:MANAGED_REGISTRY_VERSION,browser:navigator.userAgent.includes("Edg/")?"edge":"chrome",connected:true}); return; }
    validate(m);
    ...
    if (m.action === "open_stream") {
      const p=m.payload;traceManaged("B_WORKER_OPEN_RECEIVED",m,{streamerId:p.streamerId,streamSessionId:p.streamSessionId,monitorSessionId:p.monitorSessionId}); const canonicalUrl=safeUrl(p.platform,p.url); for (const item of managed.values()) if(item.streamerId===p.streamerId){const current=await chrome.tabs.get(item.tabId);const updated=await chrome.tabs.update(item.tabId,{muted:p.muted===true});traceManaged("C_TAB_UPDATED",m,{tabId:item.tabId,url:current.url});const adopted=await registry.adopt({...item,muted:Boolean(updated.mutedInfo?.muted)},{appSessionId:m.appSessionId,streamSessionId:p.streamSessionId,monitorSessionId:p.monitorSessionId});traceManaged("D_MANAGED_REGISTERED",m,{tabId:adopted.tabId,platform:adopted.platform,streamSessionId:adopted.streamSessionId,monitorSessionId:adopted.monitorSessionId});if(!await registry.get(adopted.tabId))throw new Error("MANAGED_TAB_REGISTER_FAILED");traceManaged("E_WORKER_OPENED_RESPONSE",m,{tabId:adopted.tabId});response(m,true,{...adopted,created:false,muted:adopted.muted});return;}
      const tab=await chrome.tabs.create({url:canonicalUrl,active:p.active===true}); if(typeof tab.id!=="number") throw new Error("missing_tab_id"); const updated=p.muted===false?tab:await chrome.tabs.update(tab.id,{muted:true});traceManaged("C_TAB_CREATED",m,{tabId:tab.id,url:tab.url??canonicalUrl}); const item:ManagedTab={streamerId:p.streamerId,platform:p.platform,canonicalUrl,tabId:tab.id,streamSessionId:p.streamSessionId,monitorSessionId:p.monitorSessionId,appSessionId:m.appSessionId,createdAt:Date.now(),muted:Boolean(updated.mutedInfo?.muted),createdByIntegration:true};await registry.register(item);traceManaged("D_MANAGED_REGISTERED",m,{tabId:item.tabId,platform:item.platform,streamSessionId:item.streamSessionId,monitorSessionId:item.monitorSessionId});if(!await registry.get(item.tabId))throw new Error("MANAGED_TAB_REGISTER_FAILED");traceManaged("E_WORKER_OPENED_RESPONSE",m,{tabId:item.tabId}); response(m,true,{...item,created:true}); return;
    }
    ...
    if(m.action==="configure_audio")traceManaged("H_WORKER_ENSURE_AUDIO",m,{tabId:m.payload.tabId,tabIdType:typeof m.payload.tabId});const {item,tab,diagnostic}=await exact(m);
